  ...
  return (
    <div className="space-y-4">
      ...
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-3 rounded-lg border border-border bg-card hover:bg-muted/30 transition-colors"
      >
        <div className="flex items-center gap-2">
          <TrendingUp className="w-4 h-4 text-primary" />
          <span className="text-sm font-semibold">
            {obras.length} obra(s) — clique para ver o dashboard
          </span>
        </div>
      ...
      {aberto && (
        <div className="space-y-4 animate-in fade-in duration-200">
          {obrasCategoriaData.map(({ obra, categoriasComGasto }) => {
            const percent = obra.orcamentoLimite > 0
              ? Math.min((obra.gastoAtual / obra.orcamentoLimite) * 100, 100)
              : 0;
            const overBudget = obra.orcamentoLimite > 0 && obra.gastoAtual > obra.orcamentoLimite;
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="font-medium">
                      R$ {(obra.gastoAtual || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                    </span>
                    <span className="text-muted-foreground">
                      / R$ {(obra.orcamentoLimite || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                    </span>
                  </div>
                  ...
                  <div className="text-xs font-medium mt-1" style={{ color: getStatusColor(percent) }}>
                    {obra.orcamentoLimite > 0 ? `${percent.toFixed(0)}% utilizado` : 'Sem orçamento definido'}
