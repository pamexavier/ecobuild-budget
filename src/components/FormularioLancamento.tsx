...
export function FormularioLancamento({ obras, profissionais, onSubmit }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const summary = `${obraNome} · ${selectedProf?.nome.toUpperCase()} · R$ ${valorCalculado.toFixed(2)}`;
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="rounded-lg border border-border bg-muted/20 p-4 space-y-4">
          ...
          <div>
            ...
            <select value={obraId} onChange={e => { setObraId(e.target.value); setCategoriaOrcamentoId(''); }} className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm appearance-none focus:outline-none focus:ring-2 focus:ring-ring">
          ...
          {obraId && (
            <div>
              ...
              <select value={categoriaOrcamentoId} onChange={e => setCategoriaOrcamentoId(e.target.value)} className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm appearance-none focus:outline-none focus:ring-2 focus:ring-ring">
                ...
                {categoriasObra.map(c => <option key={c.id} value={c.id}>{c.nome} — Previsto: R$ {c.valorPrevisto.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</option>)}
              </select>
            </div>
          )}
          <div>
        ...
        <div className="rounded-lg border border-border bg-muted/20 p-4 space-y-4">
          ...
          {tipo === 'diaria' && (
            <>
              <div>
                <label className="text-sm font-medium block mb-2">Turno {turnos.length > 1 && <span className="text-muted-foreground font-normal">({turnos.length} turno(s) = {((turnos.length / 2) * 100).toFixed(0)}% da diária)</span>}</label>
                <div className="flex gap-2">
                  {TURNOS.map(({ label, icon: Icon }) => (
                    <button key={label} type="button" onClick={() => toggleTurno(label)} className={`flex-1 py-2.5 rounded-lg text-sm font-medium border transition-all flex items-center justify-center gap-1.5 ${turnos.includes(label) ? 'bg-primary text-primary-foreground border-primary shadow-sm' : 'bg-background text-muted-foreground border-input hover:border-foreground/30'}`}>
              ...
              <div>
                ...
                {turnos.length > 0 && valorDiaria && <p className="text-xs text-primary mt-1.5 font-medium">Valor proporcional: R$ {valorCalculado.toFixed(2)}</p>}
              </div>
            </>
          )}
        ...
        <div className="rounded-lg border border-border bg-muted/20 p-4 space-y-3">
          ...
          <button type="button" onClick={() => fileRef.current?.click()} className="w-full rounded-lg border border-dashed border-input p-4 flex items-center justify-center gap-2 text-sm text-muted-foreground hover:bg-muted/50 transition-colors">
            <Upload className="w-4 h-4" /> {foto ? foto.name : 'Upload de foto'}
          </button>
        ...
        <Button type="submit" className="w-full py-5 text-sm font-semibold">
          <DollarSign className="w-4 h-4 mr-1" /> Lançar Pagamento
        </Button>
      </form>
    </div>
  );
