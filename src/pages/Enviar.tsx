...
const Enviar = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="bg-gradient-to-r from-[hsl(155,55%,12%)] via-[hsl(153,60%,18%)] to-[hsl(153,45%,25%)] px-5 py-6">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-white/10 flex items-center justify-center border border-white/10">
            <span className="text-xs font-black text-white tracking-tighter">ZX</span>
          </div>
          <div>
            <h1 className="text-xl font-bold text-white">ZENTRA-X</h1>
            <p className="text-sm text-white/50">Lançamento Rápido</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-5 space-y-5 max-w-lg mx-auto">
        ...
        <div>
          ...
          <select value={profissionalId} onChange={e => setProfissionalId(e.target.value)} className="w-full rounded-lg border border-input bg-card px-4 py-4 text-base appearance-none focus:outline-none focus:ring-2 focus:ring-ring">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button type="button" onClick={() => setTipo('diaria')} className={`py-4 rounded-lg text-base font-semibold border-2 transition-all flex items-center justify-center gap-2 ${tipo === 'diaria' ? 'bg-primary text-primary-foreground border-primary' : 'bg-card text-muted-foreground border-input'}`}>
              <Clock className="w-5 h-5" /> Diária
            </button>
            <button type="button" onClick={() => setTipo('empreitada')} className={`py-4 rounded-lg text-base font-semibold border-2 transition-all flex items-center justify-center gap-2 ${tipo === 'empreitada' ? 'bg-primary text-primary-foreground border-primary' : 'bg-card text-muted-foreground border-input'}`}>
              <Briefcase className="w-5 h-5" /> Empreitada
            </button>
          </div>
        </div>
          ...
          <>
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {TURNOS.map(({ label, icon: Icon }) => (
                  <button key={label} type="button" onClick={() => toggleTurno(label)} className={`py-4 rounded-lg text-sm font-semibold border-2 transition-all flex flex-col items-center gap-1 ${turnos.includes(label) ? 'bg-primary text-primary-foreground border-primary' : 'bg-card text-muted-foreground border-input'}`}>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="number" step="0.01" min="0" value={valorDiaria} onChange={e => setValorDiaria(e.target.value)} placeholder="0,00" className="w-full rounded-lg border border-input bg-card pl-12 pr-4 py-4 text-base focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
        ...
        {tipo === 'empreitada' && (
          <>
            <div>
              ...
              <input type="text" value={descricaoEtapa} onChange={e => setDescricaoEtapa(e.target.value)} placeholder="Ex: Entrega do reboco do 2º pavimento" className="w-full rounded-lg border border-input bg-card px-4 py-4 text-base focus:outline-none focus:ring-2 focus:ring-ring" />
            </div>
            <div>
              <label className="text-base font-semibold block mb-2">Valor Fixo (R$)</label>
              <div className="relative">
                <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
                <input type="number" step="0.01" min="0" value={valorEmpreitada} onChange={e => setValorEmpreitada(e.target.value)} placeholder="0,00" className="w-full rounded-lg border border-input bg-card pl-12 pr-4 py-4 text-base focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
            </div>
          </>
        )}
        ...
        <div>
          ...
          <input type="text" value={assinadoPor} onChange={e => setAssinadoPor(e.target.value)} placeholder="Nome de quem está enviando" className="w-full rounded-lg border border-input bg-card px-4 py-4 text-base focus:outline-none focus:ring-2 focus:ring-ring" />
        </div>
        ...
        <Button type="submit" className="w-full py-6 text-lg font-bold rounded-lg">
          <Send className="w-5 h-5 mr-2" /> Enviar Lançamento
        </Button>

        <p className="text-center text-xs text-muted-foreground pt-2">ZENTRA-X · Gestão Inteligente</p>
      </form>
    </div>
  );
};
